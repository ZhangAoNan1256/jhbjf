...
type (
	...
	Goods struct {
		...
		NeedPoint  int64     `db:"need_point"`  // 所需积分
...
func (m *defaultGoodsModel) Insert(ctx context.Context, data *Goods) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.GoodsName, data.NeedPoint, data.Stock)
	return ret, err
}
...
func (m *defaultGoodsModel) Update(ctx context.Context, data *Goods) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.GoodsName, data.NeedPoint, data.Stock, data.Id)
	return err
}
